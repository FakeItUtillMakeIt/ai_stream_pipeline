...
#define CUDA_CHECK(call) \
    do { \
        ...
        if (err != cudaSuccess) { \
            LOG_ERROR_FMT("[GPUResizeNormalize] CUDA error at {}:{}: {}", __FILE__, __LINE__, cudaGetErrorString(err)); \
            return; \
        } \
    } while(0)
...
// NV12转BGR的CUDA kernel
__global__ void nv12ToBGRKernel(const unsigned char* nv12,
                                 unsigned char* bgr,
    ...
    if (x < width && y < height) {
        ...
        // 读取YUV分量
        float y_val = nv12[y_idx];
        float u_val = nv12[width * height + uv_idx * 2];
        float v_val = nv12[width * height + uv_idx * 2 + 1];
        ...
        // Clamp到0-255范围
        r = fmaxf(0, fminf(255, r));
        g = fmaxf(0, fminf(255, g));
        b = fmaxf(0, fminf(255, b));

        // 写入BGR格式
        int bgr_idx = (y * width + x) * 3;
        bgr[bgr_idx + 0] = (unsigned char)b; // B
        bgr[bgr_idx + 1] = (unsigned char)g; // G
        bgr[bgr_idx + 2] = (unsigned char)r; // R
    }
}
...
// 处理GPU内存输入（零拷贝路径）
void GPUResizeNormalizeNode::processGPUMemory(std::shared_ptr<core::VideoFramePacket> frame) {
    if (!frame || !frame->gpu_data || frame->gpu_size == 0) {
        LOG_ERROR_FMT("[GPUResizeNormalize] Invalid GPU memory frame");
        return;
    }

    LOG_INFO_FMT("[GPUResizeNormalize] Processing GPU memory: data={}, size={}",
                  frame->gpu_data, frame->gpu_size);

    // 记录处理开始时间
    cudaEventRecord(start_event_, stream_);

    // 1. 获取NV12格式的GPU数据（硬件解码输出为NV12）
    void* nv12_gpu_data = frame->gpu_data;
    int src_width = frame->width;
    int src_height = frame->height;
    size_t nv12_size = src_width * src_height * 3 / 2; // NV12格式大小

    // 2. 分配GPU缓冲区用于BGR转换
    void* bgr_gpu_buffer = nullptr;
    size_t bgr_size = src_width * src_height * 3;
    CUDA_CHECK(cudaMalloc(&bgr_gpu_buffer, bgr_size));

    // 3. NV12转BGR
    dim3 block_size(16, 16);
    dim3 grid_size((src_width + block_size.x - 1) / block_size.x,
                   (src_height + block_size.y - 1) / block_size.y);

    nv12ToBGRKernel<<<grid_size, block_size, 0, stream_>>>(
        static_cast<const unsigned char*>(nv12_gpu_data),
        static_cast<unsigned char*>(bgr_gpu_buffer),
        src_width, src_height
    );

    // 4. 准备后续处理（使用BGR格式数据）
    void* input_gpu_data = bgr_gpu_buffer;
    int src_channels = 3;
    size_t src_size = bgr_size;
    ...
    if (gpu_buffer_size_ < src_size) {
        ...
        LOG_INFO_FMT("[GPUResizeNormalize] Allocated GPU buffer: {} bytes", src_size * (1 + sizeof(float)));
    }

    // 3. 复制输入数据到处理缓冲区（零拷贝优化：可以直接使用frame->gpu_data）
    // 注意：这里需要复制，因为后续会修改数据
    CUDA_CHECK(cudaMemcpyAsync(input_gpu_ptr_, input_gpu_data, src_size,
    ...
    if (status != NPP_SUCCESS) {
        LOG_ERROR_FMT("[GPUResizeNormalize] NPP resize failed: {}", int(status));
        return;
    }

    // 6. 归一化操作
    dim3 nor_block_size(16, 16);
    dim3 nor_grid_size((dst_width + nor_block_size.x - 1) / nor_block_size.x,
                   (dst_height + nor_block_size.y - 1) / nor_block_size.y);

    normalizeKernel<<<nor_grid_size, nor_block_size, 0, stream_>>>(
    ...
    CUDA_CHECK(cudaMalloc(&output_gpu_buffer, dst_size));

    // 复制归一化结果到新缓冲区
    CUDA_CHECK(cudaMemcpyAsync(output_gpu_buffer, static_cast<const void*>(output_gpu_ptr_),
                               dst_size, cudaMemcpyDeviceToDevice, stream_));
    ...
    new_packet->gpu_data = output_gpu_buffer;  // GPU内存输出
    new_packet->gpu_size = dst_size;
    new_packet->is_gpu_memory = true;  // 保持GPU内存模式
    new_packet->width = dst_width;
    new_packet->height = dst_height;
    new_packet->channels = 3;

    LOG_DEBUG_FMT("[GPUResizeNormalize] Processed GPU frame to GPU memory: {}x{}, size=%{}",
...
void GPUResizeNormalizeNode::pushData(std::shared_ptr<core::BasePacket> packet) {
    ...
    // 处理GPU内存输入（零拷贝路径）
    if (frame->is_gpu_memory && frame->gpu_data) {
        LOG_INFO_FMT("processGPUMemory");
        processGPUMemory(frame);
        return;
    }

    // 处理CPU内存输入（fallback路径）
...
void GPUResizeNormalizeNode::setGpuDeviceId(int device_id) {
    if (device_id_ != device_id) {
        ...
        cudaSetDevice(device_id_);
        LOG_INFO_FMT("[GPUResizeNormalize] Set GPU device ID: {}", device_id_);
    }
}
...
void GPUResizeNormalizeNode::setAsyncProcessing(bool async) {
    ...
    LOG_INFO_FMT("[GPUResizeNormalize] Set async processing: {}", async);
}
...
void GPUResizeNormalizeNode::setTensorRTPreprocessEnabled(bool enable) {
    ...
    LOG_INFO_FMT("[GPUResizeNormalize] Set TensorRT preprocess: {}", enable);
}
