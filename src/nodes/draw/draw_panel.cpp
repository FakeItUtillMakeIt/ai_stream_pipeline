// src/nodes/draw/draw_panel.cpp
// IDrawNode::addPanel 实现：告警信息面板绘制（从接口头文件迁出，降低头文件耦合）
...
void IDrawNode::addPanel(cv::Mat& origin, const std::vector<rules::AlertResult>& alert_results)
{
                            ...
                            int x_spacing_factor)
    {
        ...
        for (const auto& item : items) {
            ...
            if (rules::alertTypeChMap.find(item.alert_type) != rules::alertTypeChMap.end()) {
                ...
                LOG_INFO_FMT("[IDrawNode] Unknown alert type: {}", static_cast<uint8_t>(item.alert_type));
