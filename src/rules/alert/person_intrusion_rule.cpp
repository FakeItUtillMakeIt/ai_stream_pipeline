    ...
    namespace rules
    {
        ...
        bool PersonIntrusionRule::initialize(const nlohmann::json &config)
        {
            ...
            for (const auto &det_zone : intrusion_zones_)
            {
                ...
                if (!zone_is_valid)
                {
                    LOG_INFO_FMT("PersonIntrusionRule::initialize() zone {} is invalid", det_zone.first);
                    invaild_zone_count++;
                    continue;
                }
            ...
            {
                LOG_INFO("PersonIntrusionRule::initialize() all zones are invalid, global monitoring");
            }
