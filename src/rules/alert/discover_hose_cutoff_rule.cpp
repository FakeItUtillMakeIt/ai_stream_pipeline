...
namespace ai_stream
{
    namespace rules
    {
        ...
        bool DiscoverHoseCutoffRule::initialize(const nlohmann::json &config)
        {
            ...
            try
            {
                LOG_INFO_FMT("DiscoverHoseCutoffRule::initialize() config: {}", config.dump().c_str());
            ...
            LOG_INFO_FMT("DiscoverHoseCutoffRule::initialize() success");
            ...
            for (const auto &det_zone : intrusion_zones_)
            {
                ...
                if (!zone_is_valid)
                {
                    LOG_INFO_FMT("DiscoverHoseCutoffRule::initialize() zone {} is invalid", det_zone.first);
                    invaild_zone_count++;
                    continue;
                }
            ...
            {
                LOG_INFO("DiscoverHoseCutoffRule::initialize() all zones are invalid, global monitoring");
            }

            return true;
        }

        RuleStatus DiscoverHoseCutoffRule::process(
            std::shared_ptr<core::InferenceResultPacket> packet,
            AlertResult &alert_result,
            int64_t current_time_ms)
        {
            LOG_INFO_FMT("DiscoverHoseCutoffRule::process()");
            ...
            uint8_t zone_no, ZonePoints zone_points)
        {
            ...
            if (it == zone_alert_map_.end())
            {
                ...
                alert_target.object_ids = hose_track_ids;
