...
def add_nms_to_yolov8(onnx_path, output_path, num_classes=14,
    ...
    """
    适配 YOLOv8 导出的 (1, 4+num_classes, 8400) 格式
    
    原始输出: [batch, 18, 8400]  (4 box coords + 14 class logits)
    
    NonMaxSuppression 输入要求:
        - boxes:  [batch, num_boxes, 4]  (center_point_box=1 表示 cx,cy,w,h)
        - scores: [batch, num_classes, num_boxes]
    
    所以需要:
        - boxes_raw [1, 4, 8400] -> Transpose -> [1, 8400, 4]
        - scores_raw [1, 14, 8400] -> Sigmoid -> [1, 14, 8400] (无需transpose)
    """
    ...
    transpose_boxes = helper.make_node(
        ...
        inputs=['boxes_sliced'],
        outputs=['boxes_raw'],
        perm=[0, 2, 1],  # [1, 4, 8400] -> [1, 8400, 4]
        name=next_name('transpose_boxes'),
    )

    # ---------- 3. Slice 提取 scores [1, 14, 8400] ----------
    ...
    # center_point_box=1: boxes格式为 [cx, cy, w, h]
    ...
    # ---------- 6. 从 selected_indices [num_dets, 3] 提取各列 ----------
    ...
    # 拼接 [batch_col, box_col] -> [num_dets, 2]
    ...
    # ---------- 7. GatherND 提取结果 ----------
    # boxes_raw 现在是 [1, 8400, 4]，GatherND 用 [batch, box_idx] -> [num_dets, 4]
    ...
    # scores 是 [1, 14, 8400]，GatherND 用 [batch, class, box_idx] -> [num_dets]
    ...
    try:
        ...
        print(f"   类别数:{num_classes} | 最大检测:{max_detections} | IoU:{iou_threshold} | 分数阈值:{score_threshold}")
        return True
    except Exception as e:
        print(f"❌ 模型验证失败: {e}")
        try:
            onnx.save(model, output_path + '.debug.onnx')
            print(f"   调试模型已保存: {output_path}.debug.onnx")
        except:
            pass
        return False
...
def inference_with_nms(session, image_path, input_size=640):
    ...
    det_boxes, det_scores, det_classes = session.run(None, {input_name: input_tensor})
